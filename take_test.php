                ...
                <div class="row-fluid">
                    ...
                    <div id="block_bg" class="block">
                        <div class="navbar navbar-inner block-header">
                            ...
                            <?php
if($_GET['test'] == 'ok'){
/* $sqlp = mysqli_query("SELECT * FROM groupcode WHERE course_code = '".$row['course_code']."'"); */
$sqlp = mysqli_query($conn, "SELECT * FROM class_quiz WHERE class_quiz_id = '$class_quiz_id'")or die(mysqli_error());
$rowp = mysqli_fetch_array($sqlp);
/* mysqli_query("UPDATE students SET `time-left` = ".$rowp['time']." WHERE stud_id = '".$_SESSION['user_id']."'"); */
/* echo $rowp['time']; */
$x=0;
?>
                            <script>
                            ...
                            <form
                                ...
                                <?php
										...
										/* $rowa   = $row['quiz_id']; */
/* $sqla = mysqli_query("SELECT * FROM class_quiz WHERE course_code = '".$row['course_code']."'"); */
		...
		if($row['answer'] == $q) {
			$score= $score + 1;
                            ...
                            <a href="student_quiz_list.php<?php echo '?id='.$get_id; ?>"><i class="icon-arrow-left"></i>
                                Back</a>
                            <center>
                                <h3><br>Your score is <b><?php echo $score; ?></b> out of
                                    <b><?php echo ($x-1); ?></b><br /></h3>
                            </center>
                            <?php
...
?>
                            <br />
                            <?php
/* $sql = mysqli_query("SELECT * FROM students WHERE stud_id = '".$_SESSION['user_id']."'");
$row = mysqli_fetch_array($sql);
...
/* 	} else if(!is_null($row['grade'])){
		$sqlg = mysqli_query("SELECT * FROM groupcode WHERE course_code = '".$row['course_code']."'");
		$rowg = mysqli_fetch_array($sqlg);
		echo "You have already taken <b>".$rowg['course_title']."</b> - <b>".$rowg['course_code']."</b> test.";
	}
	if($row['grade']!=''){
		mysqli_query("UPDATE students SET `time-left` = 3600 WHERE stud_id = '".$_SESSION['user_id']."'");
